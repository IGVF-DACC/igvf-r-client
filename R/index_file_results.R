#' Create a new IndexFileResults
#'
#' @description
#' IndexFileResults Class
#'
#' @docType class
#' @title IndexFileResults
#' @description IndexFileResults Class
#' @format An \code{R6Class} generator object
#' @field @graph  list(\link{IndexFile}) [optional]
...
IndexFileResults <- R6::R6Class(
  ...
  public = list(
    ...
    `total` = NULL,
    `facets` = NULL,
    #' Initialize a new IndexFileResults class.
    #'
    #' @description
    #' Initialize a new IndexFileResults class.
    #'
    ...
    #' To JSON String
    #'
    #' @return IndexFileResults in JSON format
    #' @export
    toJSON = function() {
      IndexFileResultsObject <- list()
      if (!is.null(self$`@graph`)) {
        IndexFileResultsObject[["@graph"]] <-
          lapply(self$`@graph`, function(x) x$toJSON())
      }
      if (!is.null(self$`@id`)) {
        IndexFileResultsObject[["@id"]] <-
          self$`@id`
      }
      if (!is.null(self$`@type`)) {
        IndexFileResultsObject[["@type"]] <-
          self$`@type`
      }
      if (!is.null(self$`total`)) {
        IndexFileResultsObject[["total"]] <-
          self$`total`
      }
      if (!is.null(self$`facets`)) {
        IndexFileResultsObject[["facets"]] <-
          lapply(self$`facets`, function(x) x$toJSON())
      }
      IndexFileResultsObject
    },
    #' Deserialize JSON string into an instance of IndexFileResults
    #'
    #' @description
    #' Deserialize JSON string into an instance of IndexFileResults
    #'
    #' @param input_json the JSON input
    #' @return the instance of IndexFileResults
    #' @export
    fromJSON = function(input_json) {
      this_object <- jsonlite::fromJSON(input_json)
      if (!is.null(this_object$`@graph`)) {
        self$`@graph` <- ApiClient$new()$deserializeObj(this_object$`@graph`, "array[IndexFile]", loadNamespace("igvfclient"))
      }
...
',
    ...
    #' Validate JSON input with respect to IndexFileResults
    #'
    #' @description
    #' Validate JSON input with respect to IndexFileResults and throw an exception if invalid
    #'
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of IndexFileResults
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# IndexFileResults$unlock()
#
## Below is an example to define the print function
# IndexFileResults$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# IndexFileResults$lock()
