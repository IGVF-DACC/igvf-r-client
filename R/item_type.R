...
.parse_ItemType <- function(vals) {
  res <- gsub("^\\[|\\]$", "", "[AccessKey, AnalysisStep, AnalysisStepVersion, Award, Biomarker, Document, HumanDonor, RodentDonor, AlignmentFile, ConfigurationFile, ImageFile, IndexFile, MatrixFile, ModelFile, ReferenceFile, SequenceFile, SignalFile, TabularFile, AnalysisSet, AuxiliarySet, ConstructLibrarySet, CuratedSet, MeasurementSet, ModelSet, PredictionSet, PseudobulkSet, Gene, Image, InstitutionalCertificate, Lab, CrisprModification, DegronModification, AssayTerm, PhenotypeTerm, PlatformTerm, SampleTerm, OpenReadingFrame, Page, PhenotypicFeature, Publication, MpraQualityMetric, PerturbSeqQualityMetric, SingleCellAtacSeqQualityMetric, SingleCellRnaSeqQualityMetric, StarrSeqQualityMetric, InVitroSystem, MultiplexedSample, PrimaryCell, TechnicalSample, Tissue, WholeOrganism, Software, SoftwareVersion, Source, Treatment, User, Workflow]")
  unlist(strsplit(res, ", "))
}
