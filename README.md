...
library(remotes)
install_github("IGVF-DACC/igvf-r-client@v61.0.1")
```
...
cd igvf-r-client
R CMD build .
R CMD check igvfclient_61.0.1.tar.gz --no-manual
R CMD INSTALL --preclean igvfclient_61.0.1.tar.gz
```

To install the package from a local file:
```R
install.packages("igvfclient_61.0.1.tar.gz", repos = NULL, type = "source")
```
