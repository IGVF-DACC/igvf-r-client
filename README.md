...
library(remotes)
install_github("IGVF-DACC/igvf-r-client@v46.1.4")
```
...
cd igvf-r-client
R CMD build .
R CMD check igvfclient_46.1.4.tar.gz --no-manual
R CMD INSTALL --preclean igvfclient_46.1.4.tar.gz
```

To install the package from a local file:
```R
install.packages("igvfclient_46.1.4.tar.gz", repos = NULL, type = "source")
```
