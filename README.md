...
library(remotes)
install_github("IGVF-DACC/igvf-r-client@v78.0.0")
```
...
cd igvf-r-client
R CMD build .
R CMD check igvfclient_78.0.0.tar.gz --no-manual
R CMD INSTALL --preclean igvfclient_78.0.0.tar.gz
```

To install the package from a local file:
```R
install.packages("igvfclient_78.0.0.tar.gz", repos = NULL, type = "source")
```
