...
library(remotes)
install_github("IGVF-DACC/igvf-r-client@v52.0.0")
```
...
cd igvf-r-client
R CMD build .
R CMD check igvfclient_52.0.0.tar.gz --no-manual
R CMD INSTALL --preclean igvfclient_52.0.0.tar.gz
```

To install the package from a local file:
```R
install.packages("igvfclient_52.0.0.tar.gz", repos = NULL, type = "source")
```
