...
library(remotes)
install_github("IGVF-DACC/igvf-r-client@v53.0.4")
```
...
cd igvf-r-client
R CMD build .
R CMD check igvfclient_53.0.4.tar.gz --no-manual
R CMD INSTALL --preclean igvfclient_53.0.4.tar.gz
```

To install the package from a local file:
```R
install.packages("igvfclient_53.0.4.tar.gz", repos = NULL, type = "source")
```
